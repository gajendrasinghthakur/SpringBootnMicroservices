 ...
 */
public class NbaAxaEibCorrespondanceRequestor extends NbaAxaCorrespondenceRequestor { //PERF-APSL319
	private static String className = NbaAxaEibCorrespondanceRequestor.class.getName();
	...
	protected void addBody() throws Exception {
		...
		SOAPElement symbolUsr = eibBody.addChildElement(nameUsr);
		symbolUsr.addTextNode(getParameter("userName"));

		Name namePwd = factory.createName("password");
		SOAPElement symbolPwd = eibBody.addChildElement(namePwd);
		symbolPwd.addTextNode(getParameter("password")); 		
		
		if (! NbaUtils.isBlankOrNull(getParameter("categoryName"))) {
			Name categotyName = factory.createName("categoryName");
			SOAPElement symbolCatgoryName = eibBody.addChildElement(categotyName);
			symbolCatgoryName.addTextNode(getParameter("categoryName"));
		}
		
		if (! NbaUtils.isBlankOrNull(getParameter("docName"))) {
			Name docName = factory.createName("docName");
			SOAPElement symbolDocName = eibBody.addChildElement(docName);
			symbolDocName.addTextNode(getParameter("docName")); 		
		}
	}
	 ...
	 * @throws Exception
	 */
	protected void addBodyPDF() throws Exception {
		SOAPElement eibBody = body.addBodyElement(envelope.createName(getParameter("operation"),"xpr", "http://www.axa-equitable.com/xPression"));
		...
		soapDocName.addTextNode(getParameter("categoryName")); 
