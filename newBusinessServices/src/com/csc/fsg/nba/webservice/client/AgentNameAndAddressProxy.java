...
/**
 * This class makes a call to the Agent name and address Webservice.
...
public class AgentNameAndAddressProxy {
	private Call call;
	private URL url = null;
	private String stringURL = "http://ehrpsnt01:9082/lifews/services/Tx22800";
	private java.lang.reflect.Method setTcpNoDelayMethod;
	private String timeout = null;
	private static NbaLogger logger = null;

	public AgentNameAndAddressProxy() {
		try {
	...
	public synchronized org.w3c.dom.Element service(org.w3c.dom.Element part) throws Exception {
		...
		if (getURL() == null) {
			throw new SOAPException(Constants.FAULT_CODE_CLIENT, "A URL must be specified via AgentNameAndAddressProxy.setEndPoint(URL).");
		}
	...
	*/
	private static NbaLogger getLogger() {
		if (logger == null) {
			try {
				logger = NbaLogFactory.getLogger(AgentNameAndAddressProxy.class.getName());
			} catch (Exception e) {
				NbaBootLogger.log("AgentNameAndAddressProxy could not get a logger from the factory.");
