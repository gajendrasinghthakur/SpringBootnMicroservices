...
/**
 * This class makes a call to the Performance Plus Webservice.
...
public class AgentAdvancesChargebackRequestorProxy {
	private Call call;
	private URL url = null;
	private String stringURL = "http://danbant06:9083/lifews/services/Tx50500";
	private java.lang.reflect.Method setTcpNoDelayMethod;
	private String timeout = "30000";
	private static NbaLogger logger = null;

	public AgentAdvancesChargebackRequestorProxy() {
		try {
	...
	public synchronized org.w3c.dom.Element service(org.w3c.dom.Element part) throws Exception {
		...
		if (getURL() == null) {
			throw new SOAPException(Constants.FAULT_CODE_CLIENT, "A URL must be specified via AgentAdvanceChargebackRequestorProxy.setEndPoint(URL).");
		}
	...
	*/
	private static NbaLogger getLogger() {
		if (logger == null) {
			try {
				logger = NbaLogFactory.getLogger(AgentAdvancesChargebackRequestorProxy.class.getName());
			} catch (Exception e) {
				NbaBootLogger.log("AgentAdvanceChargebackRequestorProxy could not get a logger from the factory.");
