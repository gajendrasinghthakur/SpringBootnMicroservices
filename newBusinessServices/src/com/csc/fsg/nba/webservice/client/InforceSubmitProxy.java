...
public class InforceSubmitProxy extends NbaWebServiceAdapterBase {
  private Call call;
  private URL url = null;
  private String stringURL = "http://ehrpsnt01:9082/lifews/services/AccountEntry";
  private java.lang.reflect.Method setTcpNoDelayMethod;
  private NbaTXLife txLifeAccountEntryResp = null;
  private static NbaLogger logger = null;


  public InforceSubmitProxy()
  {
    try
    {
  ...
  public NbaTXLife invokeWebService(NbaTXLife nbATxLife) { // SPR2968
	  try {
		...
		txLifeAccountEntryResp = new NbaTXLife(Element2String(responseEle));
		  
	  } catch (Exception e) {
		 if (getLogger().isErrorEnabled())
				getLogger().logError(e);
	  }
	  return txLifeAccountEntryResp;
  }
...
//    String targetObjectURI = "http://webservices.life.fsg.csc.com";
    ...
    if(getURL() == null)
    {
      ...
      "A URL must be specified via AccountEntryProxy.setEndPoint(URL).");
    }
...
//	Check the response.
  ...
  /**
   ...
   private static NbaLogger getLogger() {
	   if (logger == null) {
		   try {
			   logger = NbaLogFactory.getLogger(InforceSubmitProxy.class.getName());
		   } catch (Exception e) {
