 ...
 * </thead>
 * <tr><td>AXAL3.7.17PC</td><td>AXA Life Phase 1</td><td>Paid Change Interface</td></tr>
...
public class AxaWSUpdatePolicyInvoker extends AxaWSInvokerBase {
	private static final String CATEGORY = "CAPSUpdateInforce";

	private static final String FUNCTIONID = "updateInforcePolicy";
	 ...
	 * @param nbaTXLife
	 */
	public AxaWSUpdatePolicyInvoker(String operation, NbaUserVO userVO, NbaTXLife nbaTXLife, NbaDst nbaDst, Object object) {
	...
	protected NbaTXLife createIssueTransaction(NbaTXRequestVO nbaTXRequest) throws NbaBaseException {
		...
		while (it.hasNext()) {
			...
			// CR60956 code synched
			if (issHolding.getId() != holdingIssue.getId()) { //non primary holding //ALS2653 Begin
				if (issHolding.getBankingCount() > 0) {
					Banking banking = issHolding.getBankingAt(0);
					if (banking != null) { //Holding with banking data
						BankingExtension bankingExt = NbaUtils.getFirstBankingExtension(banking);
						bankingExt.setAccountHolderNameCC(null);
					}
				}  else { //other non primary holding //ALS2653 End ALS3512
					// Begin AXAL3.7.17
					...
					while (relationIterator.hasNext()) {
						...
						// CR60956 code synched
						if (relation != null
								&& (issHolding.getId().equalsIgnoreCase(relation.getOriginatingObjectID()) || 
										issHolding.getId().equalsIgnoreCase(relation.getRelatedObjectID()))) {							
	...
	private void processFinancialActivities(Policy policyIssue,NbaOLifEId olifeId) {
		...
		boolean isPermProduct = AxaUtils.isPermProduct(getNbaTXLife().getPolicy().getProductType()); //CR60956
		while (it.hasNext()) {
			...
			if (issFinancialActivity.getFinActivityType() == NbaOliConstants.OLI_FINACT_CWAREVERSAL
				...
				it.remove();
			} else if (!isPermProduct){ //CR60956
				financialActivityList.add(issFinancialActivity);
				it.remove();
			}
		}
		if(!isPermProduct) { //CR60956
	...
	protected void addPolicyProduct(OLifE olifeIssue, Holding holdingIssue) throws NbaBaseException {
		try {
			if (holdingIssue.hasPolicy()) {
				...
				//NBA237--Modified NBAProduct to AccelProduct
	...
	private void addModalFinancialActivities(List financialActivityList, Policy policyIssue,NbaOLifEId olifeId) {
		...
		long modalPayments = 1; //APSL2640
		if((policyIssue.getApplicationInfo().getCWAAmt() - policyIssue.getPaymentAmt()) > 0) { //APSL2640
			 //overage condition
			modalPayments = new Double(Math.floor(policyIssue.getApplicationInfo().getCWAAmt() / policyIssue.getPaymentAmt())).longValue();
		}
		...
		for (int i = 0; i < modalPayments; i++) {
			...
			// ALS2655 Begin, CR60956 code synched
	...
	public void modify103Request(NbaTXLife issueTxLife) throws NbaBaseException {
		...
		try {
			if (holdingIssue != null && holdingIssue.hasPolicy()) {
				...
				AccelProduct nbaprod = npa.doProductInquiry(issueTxLife); //NBA237
				// CR60956 Code deleted
				...
				if (policyProduct != null) {
					...
					policyProductExtension = AccelProduct.getFirstPolicyProductExtension(policyProduct) ;
				}
				...
				if (life != null) {
					...
					for (int i = 0; i < count; i++) {
						...
						//Begin CR60956 Code synched with submitPolicy method
