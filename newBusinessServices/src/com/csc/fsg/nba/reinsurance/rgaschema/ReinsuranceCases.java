...
/**
 * ReinsuranceCases class.
 *
 *
 * Wrapper XSD for RGA format to include base 64 encoded file attachments
 */
public class ReinsuranceCases
	...
	/**
	 * Constant for "Cases" node name.
	 */
	public static final String $CASES = "Cases";

	/**
	 * Constant for "ReinsuranceCases" node name.
	 */
	public static final String $REINSURANCE_CASES = "ReinsuranceCases";


	/**
	 * Declarations for the XML related fields.
	 */
	protected Cases _Cases = null;
	protected java.util.ArrayList _AttachedFile = new java.util.ArrayList();
	 ...
	 */
	public ReinsuranceCases (XmlElement xml) {
		unmarshal(xml);
	}
	 ...
	 */
	public ReinsuranceCases (XmlElement xml, XmlObject parent) {
		_parent_ = parent;
		unmarshal(xml);
	}
	 ...
	 */
	public void setCases (Cases obj) {
		_Cases = obj;
	}


	private void setCases (XmlElement xml) {
	...
	/**
	 * Checks for whether Cases is set or not.
	 *
	 * @return true if Cases is set, false if not
	 */
	...
	/**
	 * Get the AttachedFile property.
	 */
	public java.util.ArrayList getAttachedFile () {
		return (_AttachedFile);
	}


	public AttachedFile getAttachedFileAt (int index)
			throws IndexOutOfBoundsException {
		return ((AttachedFile)_AttachedFile.get(index));
	}


	/**
	 * Get the count of elements in the AttachedFile property.
	 */
	public int getAttachedFileCount () {
		...
		return (_AttachedFile.size());
	}


	/**
	 * Set the AttachedFile property.
	 */
	public void setAttachedFile (java.util.ArrayList newList) {

		if (newList == null) {
			_AttachedFile.clear();
		} else {
			_AttachedFile = (java.util.ArrayList)newList.clone();
		}
	}


	public void addAttachedFile (AttachedFile obj) {
		if (obj == null) {
			return;
		}

		_AttachedFile.add(obj);
	}


	public void setAttachedFileAt (AttachedFile obj, int index)
			throws IndexOutOfBoundsException {
		if (obj == null) {
			return;
		}

		_AttachedFile.set(index, obj);
	}


	public void removeAttachedFile (AttachedFile obj) {
		if (obj == null) {
			return;
		}

		_AttachedFile.remove(obj);
	}


	public void removeAttachedFileAt (int index)
			throws IndexOutOfBoundsException {
		_AttachedFile.remove(index);
	}


	private void setAttachedFile (XmlElement xml) {

		_AttachedFile.clear();
		XmlElement saved_xml = xml;

		while (xml != null &&
			AttachedFile.matches(xml, this)) {
			Object obj = new AttachedFile(xml, this);
			_AttachedFile.add(obj);
	...
	protected String _node_name_ = $REINSURANCE_CASES;
	...
	transient protected static XmlValidator 
		_Cases_validator_ = null;
	transient protected static XmlValidator 
		_AttachedFile_validator_ = null;
	...
	protected static synchronized void createValidators () {
		...
		_Cases_validator_ = new XmlValidator(
			"ReinsuranceCases.Cases", "Element", 
			"ReinsuranceCases/Cases", 1, 1);

		_AttachedFile_validator_ = new XmlValidator(
			"ReinsuranceCases.AttachedFile", "Element", 
			"ReinsuranceCases/AttachedFile", 0, -1);
	...
	public java.util.Vector getValidationErrors (
		...
		is_valid = _Cases_validator_.isValid(
			_Cases,
		...
		is_valid = _AttachedFile_validator_.isValid(
			_AttachedFile,
			errors, return_on_error, traverse);
		if (!is_valid && return_on_error) {
			return (errors);
	...
	public static boolean matches (XmlElement xml, XmlObject parent) {
		...
		return (xml.matches($REINSURANCE_CASES, nsm, parent));
	}
	 ...
	 */
	public static ReinsuranceCases unmarshal (
			java.io.InputStream in) throws Exception {

		ReinsuranceCases obj = new ReinsuranceCases();
	...
	public void unmarshal (XmlElement xml) {
		...
		if (!xml.matches($REINSURANCE_CASES, this)) {
			return;
		}
		...
		if (xml.matches($ATTACHED_FILE,
			AttachedFile.nsm, this)) {
