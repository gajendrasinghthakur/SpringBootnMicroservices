...
public class CalcError
	...
	/**
	 * Constant for "PropertyName" node name.
	 */
	public static final String $PROPERTY_NAME = "PropertyName";

	/**
	 * Constant for "RefField" node name.
	 */
	...
	protected String _RefField = null;
	 ...
	 */
	public CalcError (XmlElement xml) {
		unmarshal(xml);
	}
	 ...
	 */
	public CalcError (XmlElement xml, XmlObject parent) {
		_parent_ = parent;
		unmarshal(xml);
	}
	...
	/**
	 * Checks for whether Message is set or not.
	 *
	 * @return true if Message is set, false if not
	 */
	...
	/**
	 * Set the RefField property.
	 */
	public void setRefField (String newValue) {
		_RefField = newValue;
	}


	/**
	 * Checks for whether RefField is set or not.
	 *
	 * @return true if RefField is set, false if not
	 */
	 public boolean hasRefField () {
		return (_RefField != null);
	}


	/**
	 * Discards RefField's value.
	 */
	 public void deleteRefField () {
		_RefField = null;
	}
	...
	/**
	 * Checks for whether PropertyName is set or not.
	 *
	 * @return true if PropertyName is set, false if not
	 */
	...
	protected String _node_name_ = $CALC_ERROR;
	...
	transient protected static XmlStringValidator 
		_Message_validator_ = null;
	transient protected static XmlStringValidator 
		_RefField_validator_ = null;
	transient protected static XmlStringValidator 
		_PropertyName_validator_ = null;
	...
	protected static synchronized void createValidators () {
		...
		_Message_validator_ = new XmlStringValidator(
			"CalcError.Message", "Element", 
			"CalcError/Message", -1, -1, 1, 1);

		_RefField_validator_ = new XmlStringValidator(
			"CalcError.RefField", "Element", 
			"CalcError/RefField", -1, -1, 1, 1);

		_PropertyName_validator_ = new XmlStringValidator(
			"CalcError.PropertyName", "Element", 
			"CalcError/PropertyName", -1, -1, 1, 1);
	...
	public java.util.Vector getValidationErrors (
		...
		e = _Message_validator_.validate(
			_Message);
		if (e != null) {
			errors.addElement(e);
			if (return_on_error) {
		...
		e = _RefField_validator_.validate(
			_RefField);
		if (e != null) {
			errors.addElement(e);
			if (return_on_error) {
		...
		e = _PropertyName_validator_.validate(
			_PropertyName);
		if (e != null) {
			errors.addElement(e);
			if (return_on_error) {
	...
	public static boolean matches (XmlElement xml, XmlObject parent) {
		...
		return (xml.matches($CALC_ERROR, nsm, parent));
	}
	 ...
	 */
	public static CalcError unmarshal (
			InputStream in) throws Exception {

		CalcError obj = new CalcError();
	...
	public void unmarshal (XmlElement xml) {
		...
		if (!xml.matches($CALC_ERROR, this)) {
			return;
		}
		...
		if (xml.matches($MESSAGE, this)) {
			setMessage(xml.getData());
			xml = xml.next();
			if (xml == null) {
				return;
			}
		...
		if (xml.matches($REF_FIELD, this)) {
			setRefField(xml.getData());
			xml = xml.next();
			if (xml == null) {
				return;
			}
		...
		if (xml.matches($PROPERTY_NAME, this)) {
			setPropertyName(xml.getData());
			xml = xml.next();
			if (xml == null) {
				return;
			}
