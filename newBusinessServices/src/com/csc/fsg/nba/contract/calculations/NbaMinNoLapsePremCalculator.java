...
/**
 * NbaMinNoLapsePremCalculator
 * A VPMS model, determined by the NbaCalculationsControlModel, is called to
 * calculate the minimun no lapse premium.  The attributes to be passed to the
 ...
 */
public class NbaMinNoLapsePremCalculator extends NbaContractCalculatorCommon implements NbaContractCalculator {
	/**
	 * Constructor for NbaMinNoLapsePremCalculator.
	 * @param aCalcType
	 * @param aNbaTxLife
	 * @throws NbaBaseException
	 */
	public NbaMinNoLapsePremCalculator(String aCalcType, NbaTXLife aNbaTxLife) throws NbaBaseException {
		super(aCalcType, aNbaTxLife);
	}
	/**
	 * Performs the calculation integration for Minimum No Lapse Premium for a contract
	...
	public NbaCalculation calculate() throws NbaBaseException, NbaVpmsException {
		try {
			if (noModelForCalc()) {
				...
				setDynamicAttribute(ATR_CALLING_PROGRAM, NbaMinNoLapsePremCalculator.class.getName());
	...
	/**
	 * Perform the minimum no lapse premium calculations to store on the base coverage.
	 */
