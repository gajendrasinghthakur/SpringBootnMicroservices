package com.csc.fsg.nba.assembler.tx151;
 ...
 */
public class TX151Assembler extends AccelTransformation {
		...
		if (request instanceof WebServiceCall) {
			try {
			    ...
			    if (getLogger().isDebugEnabled()) {
					getLogger().logDebug("TX151Assembler Raw Request: " + payload.toString());
				}
				payload = payload.substring(payload.indexOf("<TXLifeRequest"), payload.indexOf("</TXLife>") + 9);
				payload = "<TXLife xmlns=\"http://ACORD.org/Standards/Life/2\">" + payload;
				try {
					nbaTXLife = new NbaTXLife(payload);
				} catch (Exception e) {
			...
			return result;
				...
				addExceptionMessage(result, exp);
			}
		}
	...
	protected static NbaLogger getLogger() {
		if (logger == null) {
			try {
				...
				NbaBootLogger.log("TX151Assembler could not get a logger from the factory.");
