package com.csc.fsg.nba.assembler.tx1122;
 ...
 */
public class TX1122Assembler extends AccelTransformation {
		...
		if (request instanceof WebServiceCall) {
			...
			StringBuffer TXLifeStr = new StringBuffer(payload);
			  if (getLogger().isDebugEnabled()) {
					getLogger().logDebug("TX1122Assembler Raw Request: " + payload);
				}
			Result result = new AccelResult();
			NbaTXLife txLife=xsltTransformation(TXLifeStr,result);
			result.addResult(txLife);
			return result;
		}
	...
	protected NbaTXLife xsltTransformation(StringBuffer txlife,Result result)
 {
		NbaTXLife nbaTXLife = new NbaTXLife();
		...
		try {
			String txLifeStr = new String(txlife);
			txLifeString = txlife;
			...
			x.setParameter("BackEndSystem",
					NbaConfiguration.getInstance().getBusinessRulesAttributeValue(NbaConfigurationConstants.DEFAULT_INTEGRATION));
			...
			TXLife txLifedoc = null;
			ByteArrayInputStream inputstream = new ByteArrayInputStream(outputStream.toString().getBytes());
			txLifedoc = TXLife.unmarshal(inputstream);
			txLifedoc.getUserAuthRequestAndTXLifeRequest().getUserAuthRequest().getVendorApp().getVendorName().setPCDATA(providerID);
			nbaTXLife.setTXLife(txLifedoc);
		} catch (Exception exp) {
