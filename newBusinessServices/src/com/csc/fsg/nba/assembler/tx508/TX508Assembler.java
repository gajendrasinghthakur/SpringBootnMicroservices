package com.csc.fsg.nba.assembler.tx508;
 ...
 */
public class TX508Assembler extends AccelTransformation {
	...
	public Result disassemble(Object request) throws RuntimeException {
		...
		if (request instanceof WebServiceCall) {
			...
			getLogger().logDebug("ProcessACH508 Raw xml=" + TXLifeStr.toString());
			}
			...
			try {
				nbaTXLife = new NbaTXLife(TXLifeStr.toString());
				result.addResult(nbaTXLife);
			} catch (Exception e) {
	...
	private static NbaLogger getLogger() {
		if (logger == null) {
			try {
				...
				NbaBootLogger.log("TX508Assembler could not get a logger from the factory.");
