package com.csc.fsg.nba.assembler.tx203;
...
/**
 * Transformation service for preparing the <code>TXLife</code> classes for consumption
 * by an accelerator service.
 * <p>
 ...
 */
public class TX203WebServiceAssembler extends AccelTransformation {
	...
	/**
	 * Assemble the TXLife by inflating a hierarchy of Objects from the List using parent identifiers 
	 * of the Objects to associate Parents and children.
	 * @param request - List containing the a vendor String at [0] and the TXLife at [1]
	 ...
	 * @throws InvocationTargetException
	 */
	protected void createWrapperObject(List hierarchy, AccelDataObject parentObject, Class parentClass, Field aField, String tempName) throws ClassNotFoundException, NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException {
		String methodName;
		Method aMethod;
	...
	/**
	 * Disassemble a TXLife by flattening a hierarchical TXLife object into a List of objects.
	 * Disassemble a WebServiceCall by creating a Transform data object and moving the WebServiceCall.payload to Transform.payload. 
	 * @param request - List containing the a vendor String at [0] and the TXLife at [1]
	 * @return Result containing the vendor String and the flattened TXLife 
	 * @see com.csc.fs.accel.AccelTransformation#disassemble(java.lang.Object)
	 */
	public Result disassemble(Object request) throws RuntimeException {
		...
		if (request instanceof WebServiceCall) {
			...
			if (getLogger().isDebugEnabled()) {
				getLogger().logDebug("TX203WebServiceAssembler Request: " + payload);
			}
			String start_txlife="<TXLife xmlns=\"http://ACORD.org/Standards/Life/2\" Version=\"2.9.03\">";
			StringBuffer TXLifeStr = new StringBuffer(payload);
			//Remove the EIB headers and trailers
			String TXLifeStr1=TXLifeStr.substring(TXLifeStr.indexOf("<UserAuthRequest>"), TXLifeStr.indexOf("</in0>"));
			NbaTXLife aTXLife=null;
			String finalInput=start_txlife+TXLifeStr1;
			try {
				aTXLife = new NbaTXLife(finalInput);
			} catch (Exception e) {
				e.printStackTrace();
			}
			result.addResult(aTXLife);
			return result;
		}
	...
	/** 
	 * Add AccelDataObject objects found in the hierarchy of anObject to returnData, bypassing any objects for which
	 * the AccelDataObject is just a wrapper class (containing no elementary data of its own).
	 * This logic uses the xxGhost objects of the parent objects to detect which child objects are in-memory to prevent database 
	 * calls from artificially inflating the list of returned objects.  
	 * @param returnData - the List of objects to be returned
	 * @param anObject - the current node in the hierarchy
	 * @param previousObjects - previously processed objects to prevent "self" references from looping 
	 * @throws NoSuchMethodException
	 * @throws InvocationTargetException
	 * @throws IllegalAccessException
	 ...
	 */
	protected final void examineObjectFields(List returnData, AccelDataObject parentObject, AccelDataObject anObject, Set previousObjects) throws IllegalArgumentException,
					...
					if (objValue != null && !previousObjects.contains(objValue)) {
						if (objValue instanceof AccelDataObject) {		//Recurse through it's fields
							...
							while (it.hasNext()) {
								...
								if (listObjValue instanceof AccelDataObject && !previousObjects.contains(listObjValue)) {	//Recurse through the fields in the entry
	 ...
	 * @param aTXLife - the TXLife object
	 * @return the List
	 * @throws NoSuchMethodException
	 * @throws InvocationTargetException
	 * @throws IllegalAccessException
	 ...
	 * @throws NoSuchMethodException
	 */
	protected void flattenNextObject(List returnData, AccelDataObject parentObject, AccelDataObject anObject, Set previousObjects, Object objValue) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	 ...
	 * Some child objects are nested more than one level below the parent within wrapper classes, so special logic is required
	 * to navigate to the real parent of the current (child) object.
	...
	/**
	 * Find a field in an AND/OR or OLifEExtension wrapper Parent Class.  Create the missing wrapper 
	 * Class (if it doesn't exist) and repeat the process of locating the field in the wrapper Class.
	 * @param hierarchy
	 * @param childFieldName
	 * @param parentObject
	 * @param parentClass
	 ...
	 * @param compareName - the name of the object
	 * @return true if the compareName String in included in the aField String and the aField String does not
	 * contain "_Validator_" or "Ghost" 
	 */
	protected boolean isMatchForName(Field aField, String compareName) {
		String fieldName = aField.getName().toUpperCase();
		return !fieldName.equals(OLIFEEXTENSION_UC) && fieldName.indexOf(compareName) > -1 && fieldName.indexOf(VALIDATOR) < 0 && fieldName.indexOf(GHOST_UPPERCASE) < 0;
	}
	/**
	...
	/**
	 * Create a Map of IDs. The Keys of the Map are the object identifiers. The values in the Map
	 * are Lists of objects whose parent ids point to the objects. 
	 * Set the Version to 2.8.90 in the OLifE object.
	 * @param list
	 * @return
	 */
	 ...
	 * @param pre - value to be prepended to the result
	 * @param post - value to be appended to the result
	 * @param field - Field object containing the Field Name
	 ...
	 * @param anObject - the object containing the field
	 * @param anObjectClass - the Class of the object
	...
	/**
	 * Determine if an ACORD object is potentially in-memory by examining the value returned by invoking the getXxxGhost() method
	 * on its parent object. If the value is null, it is not in memory. Because non-database objects do have getXxxGhost() methods
	 * defined on the parent, treat the absence of a method as a  potentially in-memory condition.
	 * @param anObject - the object containing the field for the ACORD object 
	 * @param anObjectClass - the Class of the containing object
	 * @param field - the field for the ACORD object 
	...
	/**
	 * Determine if the object is a wrapper class that contains only references to other compound objects.
	 * UserLoginNameAndUserPswd, PswdOrCryptPswd, VendorCode and VendorApp are not wrapper classes.
	 * @param anObject - the object
	 * @return boolean
	 */
	protected boolean isWrapperOnlyClass(Object anObject) {
		String className = anObject.getClass().getName();
		return (className.indexOf(OR) > -1 
						&& !className.equals(VENDORAPP_CLASS) 
						&& !className.equals(VENDORCODE_CLASS)
						&& !className.equals(PSWDORCRYPTPSWD_CLASS) 
						&& !className.equals(ORGANIZATION_CLASS))
				|| className.indexOf(OLIFEEXTENSION) > -1 || (className.indexOf(AND) > -1 && className.indexOf(USERLOGINNAMEANDUSERPSWD_CAMEL_CASE) < 0);
	}
	/**
	 * Determine whether a field is a potential in-memory ACORD object. Because some ACORD objects are defined as Object in the parent class, the field definition 
	 * alone cannot be used. The concrete class returned by the getXxx() method is the only value that can be trusted. This method excludes fields which are
	 * obviously not potential in-memory ACORD object classes to reduce unnecessary getXxx() method calls and prevent objects from being retrieved from
	 * the database if they are not in-memory.
	 * A field is not a potential ACORD object if:
	 * - it is defined as final
	 * - or it is identified in a set of classes to ignore
	 * - or it represents the Ghost object for the real object
	 * - or it is a potential ACORD object that is not in memory  
	 * @param aParentObject - the object containing the field for the ACORD object 
	 * @param aParentObjectClass - the Class of the containing object
	 * @param aField - the field for the ACORD object 
	 * @return a boolean which indicates whether the object is a potential in-memory ACORD object
	 * @throws IllegalArgumentException
	 * @throws IllegalAccessException
	 * @throws InvocationTargetException
	 */
	protected boolean useField(Object aParentObject, Class aParentObjectClass, Field aField) throws IllegalArgumentException,
			IllegalAccessException, InvocationTargetException {
		return !Modifier.isFinal(aField.getModifiers()) 
					&& !getClassesToIgnoreForTXLife().contains(aField.getType())
					&& !aField.getName().endsWith(GHOST) 
					&& !aField.getName().endsWith(UNDERSCORE)
	...
	private static NbaLogger getLogger() {
		if (logger == null) {
			try {
				...
				NbaBootLogger.log("TX203WebServiceAssembler could not get a logger from the factory.");
