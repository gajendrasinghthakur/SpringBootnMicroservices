...
public class NbaACLabPulseProcessor extends NbaVpmsModelProcessor {

	protected ArrayList labPulseImpairments =new ArrayList();//ACN024
	...
	public void execute() throws NbaBaseException {
		...
		impSrc = NbaConstants.LABPULSE_SRC; //ACN016
		if (performingRequirementsEvaluation()) { //SPR2652
			setPartyID(work); //ACN024
			isSuccess = callLabPulseModel();
			if (!isSuccess) {
				throw new NbaVpmsException(NbaVpmsException.VPMS_RESULTS_ERROR + NbaVpmsAdaptor.AC_LAB_PULSE);	//SPR2652
			}
			   //Do the Impairments Merging //ACN024
			  mergeImpairmentsAndAccep(labPulseImpairments,accepImpairments); //ACN016							
		}
	}
		
	/**
	 * This function is used to call the ACLabPulse model
	 * @param 
	 ...
	 */
	//ACP001 New Method
	public boolean callLabPulseModel() throws NbaBaseException {
		...
		try {
			...
			vpmsProxy = new NbaVpmsAdaptor(accessContract, NbaVpmsAdaptor.AC_LAB_PULSE); //SPR3362
			...
			if (vpmsResultsData == null) {
				...
				throw new NbaVpmsException(NbaVpmsException.VPMS_NO_RESULTS + NbaVpmsAdaptor.AC_LAB_PULSE); //SPR2652
			} //SPR2652
			xmlString = (String) vpmsResultsData.getResultsData().get(0);
			if(getLogger().isDebugEnabled()) { //SPR3290
			    getLogger().logDebug("### Results from VPMS Model:" + NbaVpmsAdaptor.AC_LAB_PULSE);
			...
			labPulseImpairments = vpmsModelResult.getImpairmentInfo(); //ACN024
		...
		// SPR2652 Code Deleted
			...
			handleRemoteException(e, NbaVpmsAdaptor.AC_LAB_PULSE); //SPR2652
		//begin SPR3362
		} finally {
		    if(vpmsProxy != null){
		        try {
