...
/**
 * Class that will take care of the processing once ACLabBloodPressure model is invoked 
...
public class NbaACLabBloodPressureProcessor extends NbaVpmsModelProcessor {

	protected ArrayList labBloodPressureImpairments = new ArrayList(); //ACN024
	...
	public void execute() throws NbaBaseException {
		...
		impSrc = NbaConstants.LABBLOODPRESSURE_SRC; //ACN016
		if (performingRequirementsEvaluation()) { //SPR2652
			setPartyID(work); //ACN024
			isSuccess = callLabBloodPressure();
			if (!isSuccess) {
				throw new NbaVpmsException(NbaVpmsException.VPMS_RESULTS_ERROR + NbaVpmsAdaptor.AC_LAB_BLOODPRESSURE);	//SPR2652
			}
			   //Do the Impairments Merging //ACN024
			  mergeImpairmentsAndAccep(labBloodPressureImpairments,accepImpairments); //ACN016							
		}
	}
	/**
	 * This function is used to call the ACLabBloodPressure model
	 * @param 
	 ...
	 */
	//ACP001 New Method
	public boolean callLabBloodPressure() throws NbaBaseException {
		...
		try {
			...
			vpmsProxy = new NbaVpmsAdaptor(accessContract, NbaVpmsAdaptor.AC_LAB_BLOODPRESSURE); //SPR3362
			...
			if (vpmsResultsData == null) {
				...
				throw new NbaVpmsException(NbaVpmsException.VPMS_NO_RESULTS + NbaVpmsAdaptor.AC_LAB_BLOODPRESSURE); //SPR2652
			} //SPR2652
			xmlString = (String) vpmsResultsData.getResultsData().get(0);
			if(getLogger().isDebugEnabled()) { //SPR3290
			    getLogger().logDebug("### Results from VPMS Model:" + NbaVpmsAdaptor.AC_LAB_BLOODPRESSURE);
			...
			labBloodPressureImpairments = vpmsModelResult.getImpairmentInfo();  //ACN024
			...
			//SRP3362 code deleted
		// SPR2652 Code Deleted
		} catch (RemoteException e) {	//SPR2652
			handleRemoteException(e, NbaVpmsAdaptor.AC_LAB_BLOODPRESSURE); //SPR2652
		//begin SPR3362
		} finally {
		    if(vpmsProxy != null){
		        try {
