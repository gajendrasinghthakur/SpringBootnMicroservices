...
/**
 *  AxaRetrieveLOB2 retrieves information from an NbaLob object. A static initializer 
 ...
 *  for all entries in the Map is the retrieveLobValue() of  AxaRetrieveLOB2.  This 
 *  Map of variables is returned to the NbaOinkDataAccess when the NbaLob source is 
 *  initialized.
 *
 *  While constructing the variable map for AxaOinkDataAccess, a second map is 
 ...
 */
public class AxaRetrieveSecondaryLOB {
	...
	static {
		AxaRetrieveSecondaryLOB aAxaRetrieveLOB = new AxaRetrieveSecondaryLOB();
		String thisClassName = aAxaRetrieveLOB.getClass().getName();
		try {
			Class[] lobArgs = { Class.forName("com.csc.fsg.nba.datamanipulation.NbaOinkRequest") };
			Method lobMethod = aAxaRetrieveLOB.getClass().getDeclaredMethod("retrieveLobValue", lobArgs);
			...
			for (int i = 0; i < allMethods.length; i++) {
				...
				if (aMethodName.startsWith("get")) {
					String theVariable = aMethodName.substring(3) + "LOB2";
	...
	protected static NbaLogger getLogger() {
		if (logger == null) {
			try {
				logger = NbaLogFactory.getLogger(AxaRetrieveSecondaryLOB.class.getName());
			} catch (Exception e) {
	 ...
	 * and the Method to be invoked to retrieve the variable. The method is always retrieveLobValue().
	 * @return methods
	 */
	...
	public void retrieveLobValue(NbaOinkRequest aNbaOinkRequest) {
		...
		try {
			Object retValue = aMethod.invoke(getNbaLob(), args);
			if (returnType.equals("boolean")) {
				...
				aNbaOinkRequest.addValue(((Long) retValue).longValue());
			}
		} catch (Exception e) {
