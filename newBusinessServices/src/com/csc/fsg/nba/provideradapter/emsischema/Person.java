...
public class Person
	...
	protected java.util.Date _Dob = null;
	protected String _Ssn = null;
	 ...
	 */
	public Person (XmlElement xml, XmlObject parent) {
		_parent_ = parent;
		unmarshal(xml);
	}
	 ...
	 */
	public void setDob (java.util.Date timedate) {
		_Dob = timedate;
		_format_errors.remove("Person.Dob");
	}
	...
	/**
	 * Checks for whether Ssn is set or not.
	 *
	 * @return true if Ssn is set, false if not
	 */
	...
	protected String _node_name_ = $PERSON;
	...
	transient protected static XmlStringValidator 
		_FirstName_validator_ = null;
	transient protected static XmlStringValidator 
		_LastName_validator_ = null;
	transient protected static XmlStringValidator 
		_MiddleName_validator_ = null;
	transient protected static XmlValidator 
		_Dob_validator_ = null;
	transient protected static XmlStringValidator 
		_Ssn_validator_ = null;
	...
	protected static synchronized void createValidators () {
		...
		_FirstName_validator_ = new XmlStringValidator(
			"Person.FirstName", "Element", 
			"Person/FirstName", -1, -1, 1, 1);

		_LastName_validator_ = new XmlStringValidator(
			"Person.LastName", "Element", 
			"Person/LastName", -1, -1, 1, 1);

		_MiddleName_validator_ = new XmlStringValidator(
			"Person.MiddleName", "Element", 
			"Person/MiddleName", -1, -1, 1, 1);

		_Dob_validator_ = new XmlValidator(
			"Person.Dob", "Element", 
			"Person/DOB", 1, 1);

		_Ssn_validator_ = new XmlStringValidator(
			"Person.Ssn", "Element", 
			"Person/SSN", -1, -1, 1, 1);
	...
	public java.util.Vector getValidationErrors (
		...
		e = _FirstName_validator_.validate(
			_FirstName);
		if (e != null) {
			errors.addElement(e);
			if (return_on_error) {
		...
		e = _LastName_validator_.validate(
			_LastName);
		if (e != null) {
			errors.addElement(e);
			if (return_on_error) {
		...
		e = _MiddleName_validator_.validate(
			_MiddleName);
		if (e != null) {
			errors.addElement(e);
			if (return_on_error) {
	...
	public static boolean matches (XmlElement xml, XmlObject parent) {
		...
		return (xml.matches($PERSON, nsm, parent));
	}
	 ...
	 */
	public static Person unmarshal (
	...
	public void unmarshal (XmlElement xml) {
		...
		if (xml.matches($FIRST_NAME, this)) {
			setFirstName(xml.getData());
			xml = xml.next();
			if (xml == null) {
				return;
			}
		...
		if (xml.matches($LAST_NAME, this)) {
			setLastName(xml.getData());
			xml = xml.next();
			if (xml == null) {
				return;
			}
		...
		if (xml.matches($MIDDLE_NAME, this)) {
			setMiddleName(xml.getData());
			xml = xml.next();
			if (xml == null) {
				return;
			}
	...
	public void marshal (XmlOutputStream out) {
		...
		out.write($LAST_NAME,
			_LastName);
		out.write($MIDDLE_NAME,
			_MiddleName);
		out.write($DOB,
			com.tbf.util.DateUtilities.getFormattedDate(getDob(),
			com.tbf.util.ISO8601DateFormat.EXTENDED_DATE));
		out.write($SSN,
			_Ssn);
