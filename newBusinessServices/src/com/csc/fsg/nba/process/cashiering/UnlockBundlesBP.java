...
public class UnlockBundlesBP extends NewBusinessAccelBP {

       /**
     * Called to Unlock the selected bundles.
    ...
    public Result process(Object input) {
        ...
        try {
            if (input != null) {
                ...
                result.addResult(unlockBundle(selectedBundle));
            }
        } catch (Exception e) {
    ...
    protected List unlockBundle(List selectedBundle) throws NbaBaseException {
		...
		try {
			for (int i = 0; i < selectedBundle.size(); i++) {
				...
				isLockedByUser = cashTable.determineUserLock(bundleVO);
				if (isLockedByUser) {
					//Unlock the Bundle by current user
					cashTable.unlockBundle(bundleVO);
					unlockedBundles.add(bundleVO);
				} 				
			}
