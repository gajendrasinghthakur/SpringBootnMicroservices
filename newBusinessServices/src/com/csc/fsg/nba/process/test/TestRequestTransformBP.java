...
/**
 * Prepares an incoming XML transaction request for the Test Web Service business
...
public class TestRequestTransformBP extends AccelBP {
	...
	public Result process(Object request) {
		...
		Result result = callService(ServiceCatalog.TEST_REQUEST_DISASSEMBLER, request);
		if (!result.hasErrors()) {
			result = callService(ServiceCatalog.TEST_REQUEST_ASSEMBLER, result);
		} else {
