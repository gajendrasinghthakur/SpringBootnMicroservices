...
import com.csc.fs.util.XMLFunctions;

/**
 * MessageManagerWsXML creates outbound XML messages from Mapped values or inbound
 * Mapped values from XML messages using Message Definitions.
 * <p>
...
public class MessageManagerWsXML implements MessageManager {

	protected static final String PCDATA = "PCDATA"; //$NON-NLS-1$
...
//          }
        ...
        protected List getValues(String path, String itemName) {
			...
			if (!PCDATA.equals(itemName)) {
				pathToUse = pathToUse + "/" + itemName;
			}
        ...
        private List applyInboundBlockCriteria(List nodes, MessageDefinition.InboundCriteria criteria, String referringBlockPath) {
            ...
            if (comparetoValues.size() == 0) {
                ...
                String comparetoPath = criteria.compareToXMLPath;
                ...
                if (criteria.compareToAttribute) {
                    comparetoValues.add(comparetoContext.getAttributeValue(comparetoPath, criteria.compareToXMLItem));
                } else {
                    comparetoValues.add(comparetoContext.getValue(comparetoPath, criteria.compareToXMLItem));
                }
            }
        ...
        private List getMatchingNodes(List inputNodes, MessageDefinition.InboundCriteria criteria, List comparetoValues, String passedComparePath) {
            ...
            while (nodeIterator.hasNext()) {
                ...
                if(passedComparePath == null) {
                    comparePath = criteria.compareXMLPath;
                    int nodeLoc = criteria.compareXMLPath.indexOf(currentContext.nodeName); 
                    if (nodeLoc > -1) {
                        comparePath = criteria.compareXMLPath.substring(nodeLoc);
                    }
                } else {
                ...
                if(comparePath != null && !comparePath.equals("")) {
                    ...
                    if(xmlItems != null) {
                        ...
                        while(iterator.hasNext()) {
                            ...
                            if (criteria.compareAttribute) {
                                compareValue = item.getAttributeValue(criteria.compareXMLItem);
                            } else {
                                compareValue = item.getValue(null, criteria.compareXMLItem);
                            }   
                    ...
                    if (criteria.compareAttribute) {
                        compareValue = currentContext.getAttributeValue(comparePath, criteria.compareXMLItem);
                    } else {
                        compareValue = currentContext.getValue(comparePath, criteria.compareXMLItem);
                    }   
                    // Keep if compare value matches any compareto value.
                    Iterator comparetoIter = comparetoValues.iterator();            
                    boolean matched = false;
        ...
        private List determineBlockContext(MessageDefinition.Block currentBlock, String referringBlockXMLPath) {
            ...
            // get the block path
            String tempCompareXMLPath=criteria.compareXMLPath;
            if (criteria.compareAttribute==true){
                int indx=criteria.compareXMLPath.lastIndexOf('/');
                tempCompareXMLPath=criteria.compareXMLPath.substring(0,indx);
            }
            ...
            if (tempCompareXMLPath!=null && tempCompareXMLPath.length() < blockPath.length()) {
                ...
                // criteria path.
                if (blockPath.startsWith(criteria.compareXMLPath)) {
            ...
            if (compareNodeIsParent) {
                ...
                String relativePath = blockPath.substring(criteria.compareXMLPath.length());
...
//      }
    ...
    public Object constructRequestMessage(Record requestRecord, MessageDefinition messageDef) throws ResourceException {
		...
		if (messageDef != null && messageDef.getMessageType().equals("XML")) {
			...
			xmlRequest = constructOutboundMessage(requestBlocks, requestRecord, messageDef);
		}
		LogHandler.Factory.LogDebug(xmlRequest, "Outbound Message"); 
		Element element = null;
		try {
			element = XMLFunctions.getXMLDocument(xmlRequest);
		} catch (Exception ex) {
			ResourceException rex = new ResourceException("Failed to build request DOM Element in Message Manager");
			rex.setLinkedException(ex);
			long end = System.currentTimeMillis();
			LogHandler.Factory.LogError("XMLMessageManager", "Construct Request Message[{0}] [{1}] error [{2}-{3}]", ex, new Object[] { userName,
					executionName, ex.getClass().getName(), ex.getMessage() });
			LogHandler.Factory.LogTxnPerf("XMLMessageManager", "Construct Request Message[{0}] [{1}] completed unsuccessfully :{2}", new Object[] {
					userName, executionName, new Long(end - start) });
			throw rex;
		}
		long end = System.currentTimeMillis();
		LogHandler.Factory.LogXML("XMLMessageManager", "Construct Request Message[{0}] [{1}] XML [{2}]", new Object[]{userName, executionName,xmlRequest});		
		LogHandler.Factory.LogTxnPerf("XMLMessageManager", "Construct Request Message[{0}] [{1}] completed successfully :{2}",new Object[]{userName, executionName, new Long(end-start)});
		return element;
	}
        ...
        if (currentItem != null && currentBlockContext != null) {
            if (!processForRedefinableItem(messageDef, currentItem, currentBlockContext, subRecord)) {
                ...
                if (!processForSpecialItem(currentItem, itemValue)) {
                	...
                	Object currentObj;
                	if ("".equals(itemValue)) {
						currentObj = itemValue;
					} else {
						currentObj = currentItem.getformattedValueFromString(itemValue);
					}
                    if (currentObj!=null){
...
//      }
    ...
    public void constructResponseRecord(Object response, MessageDefinition messageDef, Record resultRecord) throws ResourceException {
		...
		try {
			...
			String responsemessage = dumpDOM(element);
            ...
            if (obj instanceof MessageDefinition.Block) {
                ...
                if (block.blockRepeats) {
                    ...
                    if (block.getRecordPath()!=null){
                        ...
                        if (repeatingRecords != null && repeatingRecords.size() == 0) { //if there was at least one data object, just add the XML for the block
							if (itemBuffer.length() > 0) {
								appendItem(blockBuffer, itemBuffer, lastElementName, lastElementValue);
								itemBuffer = new StringBuffer();
							}
					        if (block.getExternalName() != null) {
					        	block = MessageLoader.getBlock(block.getExternalName());
					        }
							itemBuffer.append("<" + block.getName());
							lastElementValue = "";
							lastElementName = block.getName();
						}
                    } else {
                ...
                if ((redefItems != null) && (redefItems.size() > 0)) {
                    ...
                    if (currentItem.isAttribute()) {  
                        ...
                        // Check Alias vs AliasName to make sure Alias was set on the Message Def
                        // AliasName will return Name if alias is null, PCDATA can be can be a field name on attachmentData
                        } else if(PCDATA.equals(currentItem.getAlias())) {
                        if (!isEmptyValue(value)) {
                        	lastElementName = currentItem.getAlias();
						...
						if (value == null) {
							lastElementValue = null;	//Preserve null values to allow them to be handled correctly later 
						} else {
							lastElementValue = currentItem.getFormattedValue(value);		//Converts nulls to empty Strings
						}
    ...
    private boolean checkOutboundCriteria(MessageDefinition.Block block, Record requestRecord) {
        if (block.getOutboundCriteria() != null) {
            ...
            if (outboundCriteria.compareRecordItem != null) {
                Object compValue = getValueFromRecord((outboundCriteria.compareRecordPath != null) ? outboundCriteria.compareRecordPath : block.getRecordPath(), requestRecord,
                        outboundCriteria.compareRecordItem);
                if (compValue != null) {
                    String compareValue = String.valueOf(compValue);    //TODO-vikrant//TypeCast to String
                    if (outboundCriteria.compareToRecordItem != null) {
                        Object compToValue = getValueFromRecord((outboundCriteria.compareToRecordPath != null) ? outboundCriteria.compareToRecordPath : block.getRecordPath(), requestRecord,
                                outboundCriteria.compareToRecordItem);
                        if (compToValue != null) {
                        ...
                        boolean match = false;
                        if (outboundCriteria.compareToValues != null && outboundCriteria.compareToValues.size() > 0) {
                            for (int i = 0; i < outboundCriteria.compareToValues.size() && !match; i++) {
                                Object value = outboundCriteria.compareToValues.get(i);
